...
const Chatbot: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: "¡Hola! Soy Leonardo. ¿Listo para dominar los mercados? ¿Cómo puedo ayudarte hoy?" }
  ]);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[100]">
      ...
      {isOpen && (
        <div className="absolute bottom-20 right-0 w-[350px] md:w-[400px] h-[500px] bg-black/95 backdrop-blur-2xl rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-fade-in-up border border-white/10">
          <div className="bg-amber-500 p-4 flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center font-bold text-black italic">L</div>
            <div>
              <h4 className="text-black font-bold leading-none">Leonardo</h4>
          ...
          <div className="p-4 border-t border-white/10 bg-[#0a0a0a]/50">
            <div className="flex gap-2">
              <input 
                ...
                placeholder="Pregunta a Leonardo..."
                className="flex-1 bg-white/5 border border-white/10 rounded-full px-4 py-2 text-sm focus:outline-none focus:border-amber-500 text-white"
              />
              <button 
                onClick={handleSend}
                disabled={isLoading}
            ...
            <p className="text-[9px] text-center text-gray-500 mt-2 font-bold tracking-widest uppercase">Precision AI Support</p>
          </div>
        </div>
      )}
    </div>
